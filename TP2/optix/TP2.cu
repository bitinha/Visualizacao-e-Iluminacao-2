...
extern "C" __global__ void __closesthit__radiance()
{
    ...
    btx = normalize(make_float3(-normal.y,normal.x,0));
    tx  = normalize(cross( btx , make_float3(normal)));
    

  
    btz = normalize(make_float3(0,normal.z,-normal.y));
    tz  = normalize(cross( btz , make_float3(normal)));
